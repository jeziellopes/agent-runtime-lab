...
export class OllamaProvider implements LLMProvider {
  generate(_request: LLMRequest): Promise<LLMResponse> {
    throw new Error('OllamaProvider.generate is not implemented')
  }

  stream(_request: LLMRequest): AsyncIterable<LLMChunk> {
    throw new Error('OllamaProvider.stream is not implemented')
  }

  countTokens(_messages: Message[]): Promise<number> {
    throw new Error('OllamaProvider.countTokens is not implemented')
  }
}
