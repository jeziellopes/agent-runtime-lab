...
export class AnthropicProvider implements LLMProvider {
  generate(_request: LLMRequest): Promise<LLMResponse> {
    throw new Error('AnthropicProvider.generate is not implemented')
  }

  stream(_request: LLMRequest): AsyncIterable<LLMChunk> {
    throw new Error('AnthropicProvider.stream is not implemented')
  }

  countTokens(_messages: Message[]): Promise<number> {
    throw new Error('AnthropicProvider.countTokens is not implemented')
  }
}
